...
type textPart struct{ text, style string }
type ArtBuilder struct{ parts []textPart }

func NewArtBuilder() *ArtBuilder { return &ArtBuilder{} }

func (b *ArtBuilder) AddText(text string) *ArtBuilder {
	b.parts = append(b.parts, textPart{text, "normal"})
	return b
}
...
func (b *ArtBuilder) Build() string {
	...
	for _, p := range b.parts {
		for i, s := range render(p.text, p.style) {
			lines[i] += s
		}
	}
